...
namespace Fintrak.Presentation.WebClient.API
{
    ...
    public class LoanScheduleApiController : ApiControllerBase
    {
        ...
        public HttpResponseMessage GetScheduleRange(HttpRequestMessage request, [FromBody] MaturityParam param)
        {
            return GetHttpResponse(request, () =>
            {
                HttpResponseMessage response = null;

                LoanScheduleData[] loanschedule = _IFRSDataService.GetScheduleRange(param.RefNo, param.Date);

                // notice no need to create a seperate model object since LoanSchedule entity will do just fine
                response = request.CreateResponse<LoanScheduleData[]>(HttpStatusCode.OK, loanschedule);
