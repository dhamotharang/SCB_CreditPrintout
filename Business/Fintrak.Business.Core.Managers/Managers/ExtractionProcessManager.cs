    ...
    public class ExtractionProcessManager : ManagerBase, IExtractionProcessService
    {
        ...
        public Upload UpdateUpload(Upload upload)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                ...
                IUploadStatusRepository uploadStatusRepository = _DataRepositoryFactory.GetDataRepository<IUploadStatusRepository>();

                Upload updatedEntity = null;
                UploadStatus uplstatus = null;

                if (upload.UploadId == 0) {
                    //update the entity...     
                    updatedEntity = uploadRepository.Add(upload);

                    //construct the uplstatus to insert... 
                    uplstatus = new UploadStatus();
                    uplstatus.UploadID = updatedEntity.UploadId;
                    uplstatus.UploadItem = upload.Title;
                    uplstatus.Severity = "M";
                    uplstatus.Status = false;

                    uploadStatusRepository.Add(uplstatus);
                } else {
        ...
        #region UploadStatus operations

        [OperationBehavior(TransactionScopeRequired = true)]
        public UploadStatus UpdateUploadStatus(UploadStatus upload) {
            return ExecuteFaultHandledOperation(() => {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_SUPER_BUSINESS };
                AllowAccessToOperation(SOLUTION_NAME, groupNames);

                IUploadStatusRepository uploadRepository = _DataRepositoryFactory.GetDataRepository<IUploadStatusRepository>();

                UploadStatus updatedEntity = null;
        ...
        [OperationBehavior(TransactionScopeRequired = true)]
        public void DeleteUploadStatus(int uploadId) {
            ExecuteFaultHandledOperation(() => {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_SUPER_BUSINESS };
                AllowAccessToOperation(SOLUTION_NAME, groupNames);
        ...
        public UploadStatus GetUploadStatus(int uploadId) {
            return ExecuteFaultHandledOperation(() => {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_SUPER_BUSINESS, GROUP_BUSINESS };
                AllowAccessToOperation(SOLUTION_NAME, groupNames);
                ...
                UploadStatus uploadEntity = uploadRepository.Get(uploadId);
                if (uploadEntity == null) {
                    NotFoundException ex = new NotFoundException(string.Format("UploadStatus with ID of {0} is not in database", uploadId));
                    throw new FaultException<NotFoundException>(ex, ex.Message);
                }
        ...
        public UploadStatus[] GetAllUploadStatuss() {
            return ExecuteFaultHandledOperation(() => {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_SUPER_BUSINESS, GROUP_BUSINESS };
                AllowAccessToOperation(SOLUTION_NAME, groupNames);
                ...
                IEnumerable<UploadStatus> uploads = uploadRepository.Get().OrderBy(c => c.UploadItem).ToArray();
        ...
        #region BuildOptionsParam

        public KeyvalueModel[] BuildOptionParams(int codenum) {

            //var connectionString = IFRSContext.GetDataConnection();
            var connectionString = GetDataConnection();


            var errExps = new List<KeyvalueModel>();

            using (var con = new SqlConnection(connectionString)) {
                var cmd = new SqlCommand("scb_runupload_options_by_code", con);
                cmd.CommandType = System.Data.CommandType.StoredProcedure;
                cmd.CommandTimeout = 0;

                cmd.Parameters.Add(new SqlParameter {
                    ParameterName = "optcode",
                    Value = codenum,
                });
                ...
                SqlDataReader reader = cmd.ExecuteReader();
                {
                    while (reader.Read()) {
                        ...
                        var uploadObj = new KeyvalueModel();

                        if (reader["uploadid"] != DBNull.Value)
                            uploadObj.Key = int.Parse(reader["uploadid"].ToString());                          

                        if (reader["message"] != DBNull.Value)
                            uploadObj.Value = reader["message"].ToString();  
                        
                        errExps.Add(uploadObj);
        ...
        #region ProcessRunJob

                public string ProcessRunJob(int id) {

                    //var connectionString = IFRSContext.GetDataConnection();
                    var connectionString = GetDataConnection();

                    using (var con = new SqlConnection(connectionString)) {
                        var cmd = new SqlCommand("scb_exec_process_proc", con);
                        cmd.CommandType = System.Data.CommandType.StoredProcedure;
                        cmd.CommandTimeout = 0;
