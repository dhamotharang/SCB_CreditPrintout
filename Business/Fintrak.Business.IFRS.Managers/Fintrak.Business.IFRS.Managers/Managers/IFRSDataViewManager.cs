    ...
    public class IFRSDataViewManager : ManagerBase, IIFRSDataViewService
    {
        ...
        public BondPeriodicSchedule[] GetBondPeriodicSchedulebyRefNo(string refNo)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                ...
                BondPeriodicSchedule[] bondPeriodicScheduleEntity = bondPeriodicScheduleMapRepository.GetBondPeriodicScheduleRefNos(refNo).OrderBy(c => c.Num_Pmt).ToArray();
        ...
        public BorrowingPeriodicSchedule[] GetBorrowingPeriodicSchedulebyRefNo(string refNo)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                ...
                BorrowingPeriodicSchedule[] borrowingPeriodicScheduleEntity = borrowingPeriodicScheduleMapRepository.Get().Where(c => c.RefNo == refNo).OrderBy(c => c.Num_Pmt).ToArray();
        ...
        public BorrowingSchedule[] GetBorrowingSchedulebyRefNo(string refNo)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                ...
                BorrowingSchedule[] borrowingScheduleEntity = borrowingScheduleMapRepository.Get().Where(c => c.RefNo == refNo).OrderBy(c => c.AMSequence).ToArray();
